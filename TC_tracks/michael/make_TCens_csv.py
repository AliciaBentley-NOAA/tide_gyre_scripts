...
#.attrib['hour']
         ...
         f = open(str.lower(ens_str)+"_"+thismember+"_"+cycle+"_"+str.lower(TC_name)+".csv","wt")
         try:
            writer = csv.writer(f)
            print len(thislat)
            for i in range(len(thislon)):
               writer.writerow([thishours[i],thistime[i].strftime('%Y'),thistime[i].strftime('%m'),thistime[i].strftime('%d'),thistime[i].strftime('%H'),thispres[i],thislat[i],thislon[i],thisspeed[i],thismember])
      ...
      # Read non-UK forecast data (GFS, EC, CMC) data
         ...
         f = open(str.lower(ens_str)+"_det_"+cycle+"_"+str.lower(TC_name)+".csv","wt")
         try:
            writer = csv.writer(f)
            print len(thislat)
            for i in range(len(clon)):
               writer.writerow([thishours[i],ctime[i].strftime('%Y'),ctime[i].strftime('%m'),ctime[i].strftime('%d'),ctime[i].strftime('%H'),cpres[i],clat[i],clon[i],cspeed[i]])
         finally:
            f.close()
